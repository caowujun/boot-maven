...
public R<Page<Fire>> page(Page<Fire> page, Fire  fire)
{
try{
return success(this.iFireService.page(page, new QueryWrapper<Fire>( fire)));
} catch (Exception ex) {
...
public R<List<Fire>> listAll(Fire   fire)
{
try{
return success(this.iFireService.list(new QueryWrapper<Fire>( fire)));
} catch (Exception ex) {
return failed(ex.getMessage());
}
}
...
public R<Boolean> save(@RequestBody Fire  fire)
{
try{
return success(this.iFireService.save( fire));
} catch (Exception ex) {
return failed(ex.getMessage());
}
}
...
public R<Boolean> update(@RequestBody Fire  fire) {
try{
return success(this.iFireService.updateById( fire));
} catch (Exception ex) {
return failed(ex.getMessage());
}
}
