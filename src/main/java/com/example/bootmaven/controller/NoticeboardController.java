...
import com.example.bootmaven.vo.votransfer.NoticeboardTransfer;
...
@Resource
private NoticeboardTransfer transfer;

/**
* 分页查询所有数据
*
* @param page 分页对象
* @param noticeboard 查询实体
* @return 所有数据
*/
@GetMapping(value = "page.do")
@ApiOperation(value = "查询Noticeboard数据列表，带分页", notes = "查询Noticeboard数据列表，带分页")
public R<Page<Noticeboard>> page(Page<Noticeboard> page, Noticeboard  noticeboard)
{
try{
return success(this.iNoticeboardService.page(page, new QueryWrapper<Noticeboard>( noticeboard)));
} catch (Exception ex) {
...
public R<List<Noticeboard>> listAll(Noticeboard   noticeboard)
{
try{
return success(this.iNoticeboardService.list(new QueryWrapper<Noticeboard>( noticeboard)));
} catch (Exception ex) {
return failed(ex.getMessage());
}
}
...
* 新增数据
*
* @param noticeboard 实体对象
* @return 新增结果
*/
@PostMapping(value = "insert.do")
@ApiOperation(value = "Noticeboard数据新增", notes = "Noticeboard数据新增")
public R<Boolean> save(@RequestBody Noticeboard  noticeboard)
{
try{
return success(this.iNoticeboardService.save( noticeboard));
} catch (Exception ex) {
return failed(ex.getMessage());
}
}
...
public R<Boolean> update(@RequestBody Noticeboard  noticeboard) {
try{
return success(this.iNoticeboardService.updateById( noticeboard));
} catch (Exception ex) {
return failed(ex.getMessage());
}
}
